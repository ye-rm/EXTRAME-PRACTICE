    ...
    , ui(new Ui::MainWindow)
{
    ...
    m_pOptThread->start();
    totalCost = 0;
	updateTempThread = std::thread(&MainWindow::updateTempAndCost, this);
...
void MainWindow::initWindow()
{
    ...
    QRegularExpression regExp("^([1][6-9]|[2][0-9]|3[0-2])$");
    auto *validator = new QRegularExpressionValidator(regExp, ui->lineEdit_temperature);
...
void MainWindow::initData()
{
    ...
    ui->label_currCostData->setText(QString("0￥"));
    ui->label_allinCostData->setText(QString("0￥"));
    ui->label_workMode_hot->show();
    ui->label_workMode_cold->hide();
...
void MainWindow::slotOnClickedTurnOff()
{
    if(m_bIsTurnOn)
    {
        m_bIsTurnOn = false;
        initData();
        totalCost+=m_client->get_estimate_fee();
        m_client->clear_estimate_fee();
        slotUpdateCurrCost(0);
        slotUpdateAllCost(totalCost);
		m_client->power_off();
...
void MainWindow::slotUpdateCurrCost(double dCurrCost)
{
    ui->label_currCostData->setText(QString::fromUtf8("%1￥").arg(dCurrCost));
}

void MainWindow::slotUpdateAllCost(double dAllCost)
{
    ui->label_allinCostData->setText(QString::fromUtf8("%1￥").arg(dAllCost));
}

void MainWindow::updateTempAndCost()
{
	while (true)
	{
		std::this_thread::sleep_for(std::chrono::seconds(1));
        slotSetCurrTemp((int)m_client->get_cur_temp());
        slotUpdateCurrCost(m_client->get_estimate_fee());
	}
}